...
const Ajv2020Class = Ajv2020Default;
...
async function ajv() {
    ajvPromise ??= (async () => {
        ...
        for (const name of ['plugin.schema.json', 'mcp.schema.json']) {
...
export async function validateAgainstSchema(schemaId, document) {
    const instance = await ajv();
    const validate = instance.getSchema(schemaId);
    if (validate === undefined)
        return [`no bundled validator for ${schemaId}`];
    validate(document);
...
export function isRecognizedSchema(value) {
 ...
 */
export async function pathContainmentError(pluginRoot, value) {
    const raw = value.replace(/^\$\{PLUGIN_ROOT\}(\/|$)/, './');
    if (!raw.startsWith('./'))
        return `path "${value}" must begin with "./" (or ${'${PLUGIN_ROOT}'})`;
...
export async function validatePluginManifest(raw) {
    const errors = [];
...
export async function validateMcpJson(pluginRoot, raw, options) {
    if (typeof raw !== 'object' || raw === null)
        return { errors: ['mcp.json is not a JSON object'] };
    const record = raw;
    const errors = [];
...
export function expandPlaceholders(value, pluginRoot, pluginData, env = process.env) {
    return value.replace(/\$\{([A-Za-z_][A-Za-z0-9_]*)(?::-([^}]*))?\}/g, (match, name, fallback) => {
        if (name === 'PLUGIN_ROOT' || name === 'CLAUDE_PLUGIN_ROOT')
            return pluginRoot;
        if (name === 'PLUGIN_DATA' || name === 'CLAUDE_PLUGIN_DATA')
            return pluginData;
        return env[name] || fallback || '';
    });
}
/** Resolve an expanded plugin-relative cwd to an absolute path. */
export function resolveCwd(value, pluginRoot, pluginData) {
    if (value.startsWith('${PLUGIN_DATA}'))
        return resolve(pluginData, value.slice('${PLUGIN_DATA}'.length).replace(/^\/+/, ''));
    if (value.startsWith('${PLUGIN_ROOT}'))
        return resolve(pluginRoot, value.slice('${PLUGIN_ROOT}'.length).replace(/^\/+/, ''));
