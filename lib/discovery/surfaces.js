...
function declaredSkillDirs(root, declared) {
    ...
    for (const value of values) {
        ...
        if (isAbsoluteWithin(root, path))
            dirs.push(path);
    }
    return dirs;
}
function isAbsoluteWithin(root, candidate) {
...
export async function discoverSkills(root, errors, declared) {
    const skills = [];
...
async function parseOneSkill(file, directory, fallbackName, errors) {
...
export async function discoverMcp(root, errors) {
...
export async function countSurfaces(root, skills, mcp) {
    let hooks = 0;
...
export async function discoverLspEntries(root) {
    const entries = [];
    try {
        for (const entry of await readdir(join(root, '.claude-plugin', 'lsp'))) {
            if (entry.endsWith('.json'))
                entries.push({ name: entry.slice(0, -5), path: join(root, '.claude-plugin', 'lsp', entry) });
        }
    }
    catch {
    ...
    try {
        for (const entry of await readdir(root, { withFileTypes: true })) {
            if (!entry.isDirectory() || !/^[a-z0-9-]+(\.[a-z0-9-]+){2,}$/.test(entry.name))
                continue;
...
async function countHookEntries(path) {
    let text;
    try {
    ...
    try {
        ...
        if (typeof hooks !== 'object' || hooks === null)
            return 0;
        return Object.values(hooks).reduce((total, entries) => total + (Array.isArray(entries) ? entries.length : 0), 0);
    }
    catch {
        return 0;
    }
