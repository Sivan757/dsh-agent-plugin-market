 ...
 */
export function toMcpMounts(suite, pluginDataRoot) {
    if (suite.mcp === undefined)
...
function toMount(suite, serverKey, server, pluginDataRoot) {
    const serverName = deriveServerName(suite.id, serverKey);
    if (server.type === 'stdio')
        return { suiteId: suite.id, serverKey, config: toStdio(serverName, suite, server, pluginDataRoot) };
    if (server.type === 'streamable-http')
        return { suiteId: suite.id, serverKey, config: toHttp(serverName, suite, server, pluginDataRoot) };
    return undefined;
}
function toStdio(serverName, suite, server, pluginDataRoot) {
...
function toHttp(serverName, suite, server, pluginDataRoot) {
    const pluginData = joinInside(pluginDataRoot, suite.id);
...
function transportReason(server) {
    if (server.type === 'sse')
        return 'legacy HTTP+SSE transport is not supported by the dsh MCP client';
    return 'unsupported mcp.json server shape';
}
function joinInside(root, segment) {
 ...
 */
export function deriveServerName(suiteId, serverKey) {
    const candidate = `${sanitizeToken(suiteId)}__${sanitizeToken(serverKey)}`;
    if (candidate.length <= SERVER_NAME_MAX)
        return candidate;
