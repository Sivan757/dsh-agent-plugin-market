 ...
 */
export function parseSkillFrontmatter(text, expectedName) {
    const match = /^---\r?\n([\s\S]*?)\r?\n---(?:\r?\n|$)/.exec(text);
    if (match === null)
    ...
    if (name === undefined || typeof name !== 'string')
        return 'frontmatter name is missing or not kebab-case';
    if (expectedName !== undefined && name !== expectedName)
        return `frontmatter name "${rawName}" does not match skill directory "${expectedName}"`;
...
function lenientFrontmatter(body) {
    const record = {};
