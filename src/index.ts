/**
 * dsh-agent-plugin host entry: the 套件 (Agent Plugin) manager.
 *
...
export function apply(ctx: Context, config: Config = {}): void {
    ...
    void (async () => {
      try {
        ...
        for (const diagnostic of diagnostics) {
          ctx.logger?.warn(`[dsh-agent-plugin] suite "${diagnostic.suiteId}" mcp server "${diagnostic.serverKey}": ${diagnostic.reason}`)
        }
      } catch (error) {
      ...
      try {
        ...
        for (const diagnostic of diagnostics) {
          ctx.logger?.warn(`[dsh-agent-plugin] suite "${diagnostic.suiteId}" hooks: ${diagnostic.reason}`)
        }
      } catch (error) {
        ctx.logger?.warn(`[dsh-agent-plugin] hooks reconcile failed: ${error instanceof Error ? error.message : String(error)}`)
      }
    })()
  }
  ...
  ctx.inject(['webServer', 'loader'], (hostCtx) => {
    hostCtx.effect(() => mountSuiteRoutes(hostCtx, manager), 'dsh-agent-plugin: http routes')
  })
