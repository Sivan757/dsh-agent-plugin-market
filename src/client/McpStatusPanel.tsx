...
export function McpStatusPanel({ t }: McpStatusPanelProps): ReactNode {
  ...
  // Disabled plugin definitions are configuration inventory, not runtime MCP
  // status rows. Keep this guard in the client as well as the host aggregator
  // so an older host response cannot reintroduce them into the UI.
  ...
  }, [activeEntries, filter, search])

  return h('div', { className: css.surface },
    h('header', { className: css.header },
      ...
      h('div', { className: css.headerActions },
        visibleTotals.failed > 0
          ? h('span', { className: css.errorSummary, title: `${visibleTotals.failed} ${t('mcpFailed')}` },
              h('span', { className: css.summaryDotRed }), visibleTotals.failed,
            )
          : null,
    ...
    error !== undefined
      ...
      : loading && activeEntries.length === 0
        ...
        : filtered.length === 0
          ...
          : h('div', { className: view === 'grid' ? css.grid : css.list },
...
function McpCard({ entry, t, onClick }: { entry: McpStatusEntry; t: Translate; onClick: () => void }): ReactNode {
  return h('button', { type: 'button', className: css.card, onClick },
...
function McpSourceBadge({ kind, label, t }: { kind: 'plugin' | 'direct'; label: string; t: Translate }): ReactNode {
  return h('span', { className: kind === 'plugin' ? css.sourcePlugin : css.sourceDirect },
    h(McpSourceIcon, { kind }),
    h('span', {}, kind === 'plugin' ? label : t('mcpDirect')),
  )
}
...
function McpSourceIcon({ kind }: { kind: 'plugin' | 'direct' }): ReactNode {
  ...
  return kind === 'plugin'
    ? h('svg', common, h('path', { d: 'M6 2.5v2H4A1.5 1.5 0 0 0 2.5 6v2h2a1.5 1.5 0 1 1 0 3h-2v2A1.5 1.5 0 0 0 4 14.5h2v-2a1.5 1.5 0 1 1 3 0v2h2a1.5 1.5 0 0 0 1.5-1.5v-2h-2a1.5 1.5 0 1 1 0-3h2V6A1.5 1.5 0 0 0 11 4.5H9v-2a1.5 1.5 0 1 0-3 0Z' }))
...
function McpFilterIcon({ kind }: { kind: Filter }): ReactNode {
  ...
  return h('svg', { width: 16, height: 16, viewBox: '0 0 16 16', fill: 'none', 'aria-hidden': true }, h('path', { d: 'M2.5 5 8 2.5 13.5 5 8 7.5 2.5 5Zm0 3L8 10.5 13.5 8M2.5 11 8 13.5 13.5 11' }))
}
...
function McpDetailModal({ entry, t, onClose }: { entry: McpStatusEntry; t: Translate; onClose: () => void }): ReactNode {
  return h(Modal, {
    ...
    children: h('div', { className: css.detail },
      h('div', { className: css.detailMeta },
        h(McpSourceBadge, { kind: entry.kind, label: entry.kind === 'plugin' ? (entry.source ?? '—') : t('mcpDirect'), t }),
        h('span', { className: css.transport }, entry.transport),
      ...
      h('section', { className: css.detailSection },
        ...
        entry.tools.length === 0
          ...
          : h('div', { className: css.toolList }, entry.tools.map((tool, index) => h('div', { key: tool.name, className: css.tool },
